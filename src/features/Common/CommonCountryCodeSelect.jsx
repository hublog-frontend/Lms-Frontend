...
export default function CommonCountryCodeSelect({
  ...
  const selectedCountry = countries.find((c) => c.phone === value) || null;
    ...
    <div style={style}>
      ...
      >
        <Autocomplete
          ...
          disabled={disabled}
          getOptionLabel={(option) => ""} // ✅ allows searching by name
          onChange={(e, newValue) =>
            onChange({ target: { value: newValue ? newValue.phone : "" } })
          }
          popupIcon={null}
          openOnFocus
          renderOption={(props, option) => {
            ...
            return (
              <Box
                key={key}
                component="li"
                sx={{ "& > img": { mr: 2, flexShrink: 0, fontSize: 11 } }}
                {...optionProps}
              >
                <img
                  loading="lazy"
          ...
          //         fn: ({ state }) => {
          //           state.styles.popper.width = "100px"; // ✅ force width here
          //         },
          //       },
          //     ],
          //   },
